...
2. the test is inconclusive if $\rho = 1​$
3. the series *diverges* if $\rho>1​$ or $\rho​$ is infinite.
...
> If the power series $\sum_{n=0}^{\infty} a_{n} x^{n}=a_{0}+a_{1} x+a_{2} x^{2}+\cdots$ converges at $  x=c \neq 0  $, then it converges absolutely for all $x$ with $|x|<|c|$. If the series diverges at $  x=d $, then it converges absolutely for all $x$ with $|x|>|d|$. 
